...
public partial class verAutores : ContentPage
{
    ...
    private async void ActualizarAutor_Clicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Mensaje", "Seleccione un autor primero", "ok");
        }
    }

    private async void EliminarAutor_Clicked(object sender, EventArgs e)
    {
        var result = await DisplayAlert("Mensaje", "¿Está seguro que desea eliminar este autor?", "Sí", "No");
        ...
        {
            await DisplayAlert("Mensaje", "Seleccione Autor", "ok");
        }
    }
}
